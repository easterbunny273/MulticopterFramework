...
void loop()
{
  ...
  {
    temperature += bmp085GetTemperature(bmp085ReadUT()) / (float) iSteps;
    float readPressure = bmp085GetPressure(bmp085ReadUP());
    pressure += readPressure / (float) iSteps ;
    altitude += ((float)44330 * (1 - pow(((float) readPressure/p0), 0.190295))) / (float) iSteps;
    delay(20);
  }
